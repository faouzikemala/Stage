<?php

namespace App\Entity;

use Doctrine\ORM\Mapping as ORM;

/**
 * EventorganizationConferenceorboothattendeeExtrafields
 *
 * @ORM\Table(name="eventorganization_conferenceorboothattendee_extrafields", indexes={@ORM\Index(name="idx_conferenceorboothattendee_fk_object", columns={"fk_object"})})
 * @ORM\Entity
 */
class EventorganizationConferenceorboothattendeeExtrafields
{
    /**
     * @var int
     *
     * @ORM\Column(name="rowid", type="integer", nullable=false)
     * @ORM\Id
     * @ORM\GeneratedValue(strategy="IDENTITY")
     */
    private $rowid;

    /**
     * @var \DateTime|null
     *
     * @ORM\Column(name="tms", type="datetime", nullable=true)
     */
    private $tms;

    /**
     * @var int
     *
     * @ORM\Column(name="fk_object", type="integer", nullable=false)
     */
    private $fkObject;

    /**
     * @var string|null
     *
     * @ORM\Column(name="import_key", type="string", length=14, nullable=true)
     */
    private $importKey;

    public function getRowid(): ?int
    {
        return $this->rowid;
    }

    public function getTms(): ?\DateTimeInterface
    {
        return $this->tms;
    }

    public function setTms(?\DateTimeInterface $tms): self
    {
        $this->tms = $tms;

        return $this;
    }

    public function getFkObject(): ?int
    {
        return $this->fkObject;
    }

    public function setFkObject(int $fkObject): self
    {
        $this->fkObject = $fkObject;

        return $this;
    }

    public function getImportKey(): ?string
    {
        return $this->importKey;
    }

    public function setImportKey(?string $importKey): self
    {
        $this->importKey = $importKey;

        return $this;
    }


}
