 ...
 * Propaldet
 *
 * @ORM\Table(name="propaldet", indexes={@ORM\Index(name="idx_propaldet_fk_product", columns={"fk_product"}), @ORM\Index(name="idx_propaldet_fk_propal", columns={"fk_propal"}), @ORM\Index(name="fk_propaldet_fk_unit", columns={"fk_unit"})})
 * @ORM\Entity
 */
class Propaldet
{
     ...
     * @var \Propal
     *
     * @ORM\ManyToOne(targetEntity="Propal")
     * @ORM\JoinColumns({
    ...
    public function getFkPropal(): ?Propal
    {
        return $this->fkPropal;
    }

    public function setFkPropal(?Propal $fkPropal): self
    {
        $this->fkPropal = $fkPropal;
